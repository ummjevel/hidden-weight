...
- **동작**:
  ...
  - `BuildMovingPlatform()`/`BuildCrumblingPlatform()`: `Rigidbody2D`(Kinematic)+`BoxCollider2D`(3×0.5, `Platform.png` 96×16px/32ppu)+각각 `MovingPlatform`/`CrumblingPlatform`. 레이어는 `Ground`.
  - `BuildRewindableBlock()`: `Rigidbody2D`(기본 Dynamic 그대로 — 되감기로 밀린 위치를 되돌려야 하므로)+`BoxCollider2D`(1×1)+`Rewindable`. 레이어는 `Ground`.
...
- **주요 멤버**:
  ...
  - 배치 헬퍼: `BuildRoom`, `BuildZoneVolume`, `BuildSolidBlock`, `BuildSafePlatform`, `BuildZoneTrigger`, `BuildStoryFragment`, `BuildHiddenFragment`, `BuildGate`, `BuildEnemy`, `BuildGazeHazard`, `BuildMovingPlatform`, `BuildCrumblingPlatform`, `BuildRewindableBlock`, `BuildCheckpoint`, `BuildEventSystem`, `BuildPauseMenu`, `BuildZoneCommon(string zoneAssetName, Vector3 playerSpawn, out GameObject root)`(GameManager+MainCamera+Player+HUD+PauseMenu+EventSystem+Grid/Tilemap+지역 Volume 공통 킷).
