...
namespace HiddenWeight.Tests
{
    public class FractureTraversalAssetTests
    {
        ...
        [Test]
        public void Fracture_WallMiddle은_결정코어와_고딕세로장식이_있다()
        {
            const string assetPath =
                TerrainRoot + "/Fracture_TraversalWallMiddle_v3.png";
            string fullPath = Path.Combine(Application.dataPath, "..", assetPath);
            var texture = new Texture2D(2, 2, TextureFormat.RGBA32, false);
            Assert.IsTrue(ImageConversion.LoadImage(
                texture, File.ReadAllBytes(fullPath)), assetPath);
            ...
            Assert.Greater(brightCyan, 1000,
                assetPath + " has no readable vertical crystal core");
            Assert.Greater(warmMetal, 1500,
                assetPath + " has no readable gothic vertical frame");
            Object.DestroyImmediate(texture);
        }
