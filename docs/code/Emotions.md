...
## AwarenessSystem.cs
- **역할**: `L` 홀드 자각. 채도를 잃는 URP `Volume` 연출, 이동 감속, 씬 안 `IAwarenessReactive` 대상에 대한 On/Off 방송을 담당하는 단일 `MonoBehaviour`. **(2026-07-26 재작업)** 균열 지역(`awarenessStable == false`)에서는 자각이 **완전히 무력화**된다 — 볼륨·감속 등 "자각을 쓰는 감각"은 그대로지만 반응 오브젝트에는 항상 "이상 없음"(false)만 방송된다(기획서 EMOTION_SYSTEM 3.3절: 예지만이 유일하게 신뢰 가능한 정보원). 이전의 "절반을 무작위로 깜빡이는" `UnstableFlicker` 방식은 파편이 결국 보여서 기획 의도와 어긋나 제거했다.
...
- **동작** (2026-07-26 재작업):
  ...
  - `OnEnable`/`OnDisable`에서 `AwarenessRegistry.Added`를 구독/해제한다. `HandleAdded(IAwarenessReactive r)`는 **`_revealing`**(반응 오브젝트들에 마지막으로 방송한 값)이 `true`인 상태에서 새로 등록된 오브젝트(씬 로드 순서상 `HiddenFragment.OnEnable`이 이 시점 이후에 도는 경우 대비)에게 즉시 `OnAwarenessChanged(true)`를 호출해 동기화한다.
  - `Update`: `wanted = State == Playing && Progress.HasAwareness && PlayerInput.AwarenessHeld`를 매 프레임 계산해(`State` 조건이 식 안으로 들어옴 — Ending·일시정지 중 오동작 방지) `IsActive`와 다르면 `Activate()`/`Deactivate()`를 호출한다. 이어서 `reveal = IsActive && IsStable`을 `_revealing`과 비교해 달라졌을 때만 `Broadcast(reveal)` — **켜짐 + 안정 지역일 때만 true가 방송**되며, 자각을 유지한 채 안정 ↔ 균열 지역을 넘나드는 전환도 이 비교 한 번으로 따라온다.
  - `Activate()`/`Deactivate()`: `PlayerController.Instance.ExternalSpeedMultiplier`를 `slowMultiplier`(0.6)/1로, Volume weight를 `volumeRampTime`(0.25초)에 걸쳐 1/0으로 램프. 방송은 하지 않는다(위 `Update`의 `_revealing` 비교가 전담) — 균열에서도 채도 상실·감속은 그대로라 "자각은 되는데 아무것도 안 잡히는" 기만이 성립한다.
  ...
  - 균열 지역의 파편은 이 무력화 때문에 `HiddenFragment`가 아니라 눈에 보이는 일반 `StoryFragment`로 배치된다(숨기면 영영 찾을 수 없으므로 — `ZoneSceneBuilder.BuildZoneFracture` 참고).
...
- **동작**:
  ...
  - `OnEnd()`: 저장해둔 레이어·스케일을 복원하고, `PlayerAttack`이 있으면 `CanAttack = true`로 되돌린다. **(2026-07-26 추가)** 같은 GameObject의 `PlayerHealth.GrantInvulnerability(releaseInvulnSeconds)`(직렬화 필드, 기본 0.2초)를 호출해 원래 크기로 돌아오는 순간의 피격을 막는다(기획서 EMOTION_SYSTEM 2.2절 해제 시 무적프레임).
...
- **동작**:
  - `Start()`/`OnDestroy()` (2026-07-26 추가): 같은 GameObject의 `PlayerHealth.Damaged` 이벤트를 구독/해제한다. `HandleDamaged()`는 `IsActive`면 `End()` — **채널링 중 피격 시 즉시 캔슬 + 쿨타임만 소모**(기획서 EMOTION_SYSTEM 1.2절). `PlayerHealth`는 베이스의 `Player` 프로퍼티가 아니라 자기 자신에서 `GetComponent`로 찾는다 — 씬 단독 로드 시 컴포넌트 Awake 순서에 따라 `PlayerController.Instance`가 null/파괴 상태일 수 있기 때문(스킬은 항상 `PlayerHealth`와 같은 GameObject에 있다).
